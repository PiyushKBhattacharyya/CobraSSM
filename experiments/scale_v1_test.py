...
def scale_v1_audit():
    print("=" * 60)
    print("  COBRASSM LEVEL 1 AUDIT (~10M-15M PARAMS)")
    print("=" * 60)
    
    # 1. Configuration (Option A: Real Vocab)
    config = CobraConfig(
        vocab_size=50257,  # Standard GPT-2
        d_model=256,
        num_hidden_layers=6,
        d_state=16,
        num_scales=4
    ...
    print(f"  - Cobra Blocks (6) : {block_params:,} ({block_params/total_params:.1%})")
    ...
    if total_params < 20_000_000:
        print("  VERIFIED: Level 1 Scaling is stable on this hardware.")
    else:
        print("  WARNING: Model is larger than expected Level 1 bounds.")
    print("=" * 60)
